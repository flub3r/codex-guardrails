...
2. **Explore:** Spawn one `explorer` only when ownership, execution flow, or root cause remains unclear.
...
- The configuration caps open subagent threads at four; project rules start with one and default to no more than two investigative workers.
